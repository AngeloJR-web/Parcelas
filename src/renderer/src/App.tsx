...
// Lógica inteligente para as letras do avatar
...
function App() {
  ...
  // --- NOVO: ESTADOS DO PERFIL DO UTILIZADOR ---
  const [profile, setProfile] = useState({
    fullName: 'Angelo Oliveira',
    displayName: 'Angelo O.',
    email: 'angelo.o@email.com',
  ...
  const [hidePaidInstallments, setHidePaidInstallments] = useState(false)
  ...
  // --- Função para abrir a edição e carregar os dados atuais ---
  const handleOpenEditProfile = () => {
    setDraftProfile(profile);
    setActiveView('editar_perfil');
  }

  // --- HANDLERS DIVERSOS ---
  const handleOpenAddCardForm = () => { 
    setEditingCardId(null); setName(''); setClosingDate(''); setDueDate(''); setLimitTotal(''); setShowForm(!showForm); 
  }
  
  const handleEditCard = (card: Card) => { 
    setEditingCardId(card.id!); setName(card.name); setClosingDate(card.closingDate.toString()); setDueDate(card.dueDate.toString()); 
    setLimitTotal(formatBRL(card.limitTotal));
    setShowForm(true); 
  }

  const handleCardSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    const parsedLimit = parseCurrency(limitTotal);
    if (parsedLimit <= 0) return showToast("Valor de limite inválido!", "error");

    const newCard = { name, closingDate: Number(closingDate), dueDate: Number(dueDate), limitTotal: parsedLimit }
  ...
  const handleOpenAddPurchaseModal = () => { 
    setEditingPurchaseId(null); setPurchaseCardId(''); setPurchaseDesc(''); setPurchaseTotal(''); setPurchaseInstallments('1'); setPurchaseDate(''); setPurchaseCategory('Eletrônicos'); setIsPurchaseModalOpen(true); 
  }
  
  const handleEditPurchase = (p: any) => { 
    setEditingPurchaseId(p.id); setPurchaseCardId(p.cardId.toString()); setPurchaseDesc(p.desc); 
    setPurchaseTotal(formatBRL(p.total));
    setPurchaseInstallments(p.parcelas.toString()); setPurchaseDate(p.date); setPurchaseCategory(p.category || 'Outros'); setIsPurchaseModalOpen(true); 
  }
  ...
  const handlePurchaseSubmit = async (e: React.FormEvent) => {
    ...
    const formattedTotal = parseCurrency(purchaseTotal);
    if (formattedTotal <= 0) return showToast("Valor inválido!", "error");

    const finalInstallments = Number(purchaseInstallments) || 1;
    
    const pData = { cardId: Number(purchaseCardId), description: purchaseDesc, totalAmount: formattedTotal, installmentsCount: finalInstallments, purchaseDate: purchaseDate, category: purchaseCategory }
  ...
  const categoryColors: Record<string, string> = { 'Eletrônicos': '#5cd685', 'Casa': '#3b82f6', 'Assinaturas': '#ec4899', 'Viagem': '#f59e0b', 'Educação': '#a855f7', 'Carros': '#ef4444', 'Outros': '#6b7280' };
  ...
  const displayInstallments = hidePaidInstallments ? sortedInstallments.filter(item => item.status !== 'Pago') : sortedInstallments;
  ...
  const renderIcon = (category: string) => {
    switch (category) {
      ...
      case 'Carros': return ( <svg viewBox="0 0 24 24" fill="currentColor" width="20" height="20"><path d="M18.92 6.01C18.72 5.42 18.16 5 17.5 5h-11c-.66 0-1.21.42-1.42 1.01L3 12v8c0 .55.45 1 1 1h1c.55 0 1-.45 1-1v-1h12v1c0 .55.45 1 1 1h1c.55 0 1-.45 1-1v-8l-2.08-5.99zM6.85 7h10.29l1.08 3.11H5.77L6.85 7zM19 17H5v-5h14v5z"/><circle cx="7.5" cy="14.5" r="1.5"/><circle cx="16.5" cy="14.5" r="1.5"/></svg> );
  ...
  const draftInitials = getInitials(draftProfile.displayName);
    ...
    <div className="app-layout">
      ...
      <aside className="sidebar">
        ...
        <div className={`sidebar-profile ${activeView === 'perfil' || activeView === 'editar_perfil' ? 'active' : ''}`} onClick={() => setActiveView('perfil')}>
          ...
          <div className="profile-info"><span className="profile-name">{profile.displayName}</span><span className="profile-link">Ver perfil</span></div>
        </div>
      </aside>
              ...
              {cards.map((card) => {
                ...
                return (
                  <div key={card.id} className="bank-card" style={{ background: visuals.bg }} onClick={() => handleCardClick(card)}>
                    ...
                    <div className="card-bottom"><div><div className="card-limit-label">Limite Total</div><div className="card-limit-value">R$ {formatBRL(card.limitTotal)}</div><div className="card-date">{String(card.closingDate).padStart(2, '0')}/{String(card.dueDate).padStart(2, '0')}</div></div></div>
                  </div>
                )
              })}
            </div>
            <div className="bento-panel">
              <div className="panel-header"><h2>Gastos Mensais de {currentYear}</h2><span style={{ color: 'var(--brand-green)', fontSize: '0.85rem', fontWeight: 600 }}>Pico: R$ {formatBRL(maxSpendingInYear)}</span></div>
              <div className="chart-container">
                <svg className="svg-chart" viewBox="0 0 1000 200" preserveAspectRatio="none">
                  ...
                  <line x1="0" y1="50" x2="1000" y2="50" stroke="var(--border-color)" strokeWidth="1" /><line x1="0" y1="100" x2="1000" y2="100" stroke="var(--border-color)" strokeWidth="1" /><line x1="0" y1="150" x2="1000" y2="150" stroke="var(--border-color)" strokeWidth="1" />
                  {linePathString && ( <><path d={areaPathString} fill="url(#chartGlow)" /><path d={linePathString} fill="none" stroke="#5cd685" strokeWidth="3" /></> )}
                </svg>
                <div style={{ display: 'flex', justifyContent: 'space-between', color: 'var(--text-secondary)', fontSize: '0.8rem', marginTop: '12px' }}>{['Jan', 'Fev', 'Mar', 'Abr', 'Mai', 'Jun', 'Jul', 'Ago', 'Set', 'Out', 'Nov', 'Dez'].map((m, idx) => (<div key={m} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '60px' }}><span style={{ fontWeight: 600, color: 'var(--text-main)' }}>{m}</span><span style={{ fontSize: '0.7rem', color: monthlyTotals[idx] > 0 ? 'var(--brand-green)' : 'var(--text-secondary)' }}>R$ {formatBRL(monthlyTotals[idx])}</span></div>))}</div>
              </div>
            </div>
          </>
        )}
          ...
          <>
            ...
            {showForm && (
              <form className="inline-form" onSubmit={handleCardSubmit} style={{ marginBottom: '30px', borderColor: editingCardId ? '#a855f7' : 'var(--border-color)' }}>
                ...
                <div className="form-group"><label>Limite Total</label><input type="text" className="form-input" value={limitTotal} onChange={(e) => setLimitTotal(maskCurrency(e.target.value))} placeholder="0,00" required /></div>
                ...
                {cards.map((card) => {
                  ...
                  return (
                    <div key={card.id} className="sleek-physical-card" style={{ background: visuals.bg }} onClick={() => handleCardClick(card)}>
                      ...
                      <div className="card-bottom-row"><div className="card-limit-block"><span className="card-limit-label">Limite disponível</span><span className="card-limit-val">R$ {formatBRL(card.limitTotal)}</span></div><div>{renderFlagLogo(visuals.flag)}</div></div>
                    </div>
                  )
                })}
              </div>
            ...
            <div className="stats-bar">
              ...
              <div className="stat-item"><div className="stat-icon" style={{ background: 'rgba(168, 85, 247, 0.1)', color: '#a855f7' }}>💰</div><div className="stat-info"><span className="stat-label">Limite total</span><span className="stat-value">R$ {formatBRL(totalLimitAll)}</span></div></div>
              <div className="stat-item"><div className="stat-icon" style={{ background: 'rgba(59, 130, 246, 0.1)', color: '#3b82f6' }}>🛍️</div><div className="stat-info"><span className="stat-label">Compras em aberto</span><span className="stat-value">R$ {formatBRL(pendingPurchasesTotal)}</span></div></div>
        ...
        {activeView === 'compras' && (
          <>
            ...
            <div className="purchases-stats-row">
              ...
              <div className="purchase-stat-card"><div className="p-stat-icon" style={{ background: 'rgba(245, 158, 11, 0.1)', color: '#f59e0b' }}>✅</div><div className="p-stat-info"><span className="p-stat-label">Valor a pagar</span><span className="p-stat-value">R$ {formatBRL(valueToPayNext30Days)}</span><span className="p-stat-sub">Próximos 30 dias</span></div></div>
            </div>
                    ...
                    {purchases.map((purchase) => {
                      ...
                      return (
                        <tr key={purchase.id}>
                          ...
                          <td style={{ fontWeight: 600 }}>R$ {formatBRL(purchase.total)}</td>
                          <td><div className="parcela-cell"><span className="parcela-badge">{purchase.parcelas}x</span><span style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>R$ {formatBRL(purchase.total / purchase.parcelas)}</span></div></td>
            ...
            <div className="invoice-hint"><div className="invoice-hint-icon" style={{background: 'rgba(255,255,255,0.05)', color: 'var(--text-main)'}}>ℹ</div>Dica: Mantenha suas compras organizadas para controlar melhor seus gastos.</div>
          </>
        )}
            ...
            <div className="invoice-header-top">
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '25px' }}>
                <div className="toggle-switch-container">
                  <span className="toggle-switch-label">Ocultar pagas</span>
                  <label className="toggle-switch">
                    <input type="checkbox" className="toggle-input" checked={hidePaidInstallments} onChange={(e) => setHidePaidInstallments(e.target.checked)} />
                    <span className="toggle-slider"></span>
                  </label>
                </div>
                <div className="invoice-top-widget"><div className="widget-icon-box">💳</div><div><span style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '1px' }}>Total Próximo</span><h3 style={{ fontSize: '1.6rem', fontWeight: 700, margin: '2px 0' }}>R$ {formatBRL(nextTotal)}</h3><p style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>{nextCount} parcelas pendentes</p></div></div>
              </div>
            </div>

            <div>
              {displayInstallments.length === 0 ? ( <div style={{ textAlign: 'center', padding: '50px', color: 'var(--text-secondary)' }}><p>Nenhuma fatura pendente neste momento.</p></div> ) : (
                displayInstallments.map((item) => {
                  ...
                  return (
                    <div key={item.id} className="sleek-invoice-card" onClick={() => handleTogglePayment(item.id, item.status)} style={{ opacity: isPaid ? 0.6 : 1 }}>
                      ...
                      <div className="sleek-card-right"><div className="sleek-status-badge" style={{ backgroundColor: isPaid ? 'rgba(92, 214, 133, 0.15)' : 'rgba(59, 130, 246, 0.15)', color: isPaid ? '#5cd685' : '#3b82f6' }}><span className="dot" style={{ backgroundColor: isPaid ? '#5cd685' : '#3b82f6' }}></span>{item.status}</div><div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', marginTop: '10px' }}><span style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>Total</span><strong style={{ fontSize: '1.3rem', fontWeight: 700, color: 'var(--text-main)' }}>R$ {formatBRL(item.amount)}</strong></div></div>
        ...
        {activeView === 'relatorios' && (
          <>
            <div className="view-header">
              ...
              <div className="date-picker-mock" onClick={() => monthPickerRef.current?.showPicker()} style={{ position: 'relative', cursor: 'pointer', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', padding: '10px 18px', borderRadius: '10px', color: 'var(--text-secondary)' }}>
                ...
                <div className="donut-container">
                  <div style={donutChartStyleReport}>
                    <div style={{ width: '170px', height: '170px', borderRadius: '50%', backgroundColor: 'var(--bg-panel)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                      ...
                      <strong style={{ fontSize: '1.5rem', color: 'var(--text-main)', margin: '4px 0' }}>R$ {formatBRL(totalPurchasesReportValue)}</strong>
                    ...
                    return (
                      <div key={cat} className="cat-list-row">
                        ...
                        <div className="cat-right">
                          ...
                          <span className="cat-val">{amount > 0 ? `R$ ${formatBRL(amount)}` : 'R$ 0,00'}</span>
            ...
            <div className="reports-bottom-stats">
              ...
              <div className="r-stat-card"><div className="r-stat-icon" style={{ background: 'rgba(168, 85, 247, 0.1)', color: '#a855f7' }}>📈</div><div className="r-stat-info"><span className="r-label">Ticket médio</span><span className="r-value">R$ {formatBRL(averageTicket)}</span><span className="r-sub">por compra</span></div></div>
        ...
        {activeView === 'perfil' && (
          <>
            <div className="profile-header-top">
              ...
              <button className="btn-edit-profile" onClick={handleOpenEditProfile}>✏️ Editar perfil</button>
            </div>

            <div className="profile-layout-grid">
              <div className="profile-main-card">
                <div className="profile-avatar-large-container">
                  <div className="profile-avatar-large">{profileInitials}<div className="edit-avatar-badge" onClick={handleOpenEditProfile}>✏️</div></div>
                  <h2>{profile.displayName}</h2>
                  <div className="status-badge"><span className="dot"></span> Conta ativa</div>
                </div>

                <div className="profile-details-list">
                  <div className="p-detail-item"><div className="p-detail-left"><span>📧</span> {profile.email}</div></div>
              ...
              <div className="profile-right-column">
                <div>
                  ...
                  <div className="p-summary-grid">
                    ...
                    <div className="p-summary-card"><div className="p-s-icon" style={{ background: 'rgba(245, 158, 11, 0.1)', color: '#f59e0b' }}>📈</div><span className="p-s-value">R$ {formatBRL(valueToPayNext30Days)}</span><span className="p-s-label">Total a pagar</span><span className="p-s-sub">Próximos 30 dias</span></div>
                  </div>
                </div>
                  ...
                  <div className="p-sub-panel">
                    ...
                    <div className="activity-timeline">
                      <div className="activity-item">
                        <div className="activity-line"></div><div className="act-icon" style={{background: 'rgba(92, 214, 133, 0.15)', color: '#5cd685', borderColor: 'var(--border-color)'}}>✓</div>
                      ...
                      <div className="activity-item">
                        <div className="activity-line"></div><div className="act-icon" style={{background: 'rgba(59, 130, 246, 0.15)', color: '#3b82f6', borderColor: 'var(--border-color)'}}>💳</div>
                      ...
                      <div className="activity-item">
                        <div className="activity-line"></div><div className="act-icon" style={{background: 'rgba(168, 85, 247, 0.15)', color: '#a855f7', borderColor: 'var(--border-color)'}}>📈</div>
                      ...
                      <div className="activity-item">
                        <div className="activity-line"></div><div className="act-icon" style={{background: 'rgba(245, 158, 11, 0.15)', color: '#f59e0b', borderColor: 'var(--border-color)'}}>🔔</div>
          ...
          <>
            <div className="edit-header-top">
              <div className="edit-header-left">
                <button className="btn-back" onClick={() => { setDraftProfile(profile); setActiveView('perfil'); }}>❮</button>
                <div className="edit-header-title">
                  <h1>Editar Perfil</h1>
                  <p>Atualize suas informações e preferências</p>
                </div>
              </div>
              <button className="btn-view-profile" onClick={() => { setDraftProfile(profile); setActiveView('perfil'); }}>👁️ Ver perfil</button>
            </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '25px' }}>
                ...
                <div className="edit-panel">
                  ...
                  <div className="edit-form-grid">
                    <div className="edit-input-group">
                      ...
                      <input type="text" className="edit-input" value={draftProfile.fullName} onChange={(e) => setDraftProfile({...draftProfile, fullName: e.target.value})} />
                    </div>
                    <div className="edit-input-group">
                      <label>Nome de exibição</label>
                      <div className="edit-input-wrapper">
                        <input type="text" className="edit-input has-icon-right" value={draftProfile.displayName} onChange={(e) => setDraftProfile({...draftProfile, displayName: e.target.value})} />
                    ...
                    <div className="edit-input-group">
                      <label>E-mail</label>
                      <div className="edit-input-wrapper">
                        <input type="email" className="edit-input has-icon-right" value={draftProfile.email} onChange={(e) => setDraftProfile({...draftProfile, email: e.target.value})} />
                    ...
                    <div className="edit-input-group">
                      ...
                      <div className="edit-input-wrapper">
                        <input type="text" className="edit-input has-icon-right" value={draftProfile.birthDate} onChange={(e) => setDraftProfile({...draftProfile, birthDate: e.target.value})} />
                        <span className="edit-input-icon-right" style={{color: 'var(--text-secondary)'}}>📅</span>
                      </div>
                    </div>
                    <div className="edit-input-group">
                      ...
                      <div className="edit-input-wrapper">
                        ...
                        <input type="text" className="edit-input has-icon-left" value={draftProfile.phone} onChange={(e) => setDraftProfile({...draftProfile, phone: e.target.value})} />
                      </div>
                    </div>
                    <div className="edit-input-group">
                ...
                <div className="edit-actions-bar">
                  ...
                  <div className="edit-actions-right">
                    <button className="btn-cancel" onClick={() => { setDraftProfile(profile); setActiveView('perfil'); }}>Cancelar</button>
                    <button className="btn-save" onClick={() => { setProfile(draftProfile); showToast("Perfil atualizado com sucesso!"); setActiveView('perfil'); }}>✓ Salvar alterações</button>
                  </div>
                </div>
          ...
          <div className="card-details-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="card-details-header">
              ...
              <div className="card-details-stats">
                <div className="stat-block"><span className="value green">R$ {formatBRL(overlayStats.total)}</span><span className="label">em parcelas</span></div>
                    ...
                    overlayInstallments.map((item) => {
                      ...
                      return (
                        <tr key={item.id} style={{ opacity: isPaid ? 0.5 : 1 }}>
                          ...
                          <td style={{ fontWeight: 600 }}>R$ {formatBRL(item.amount)}</td>
      ...
      {/* MODAL PADRÃO DE COMPRA TOTAL COM MÁSCARA */}
      {isPurchaseModalOpen && (
          ...
          <div className="modal-container">
            ...
            <form className="modal-form" onSubmit={handlePurchaseSubmit}>
              ...
              <div className="modal-input-group">
                ...
                <input type="text" className="form-input" placeholder="0,00" value={purchaseTotal} onChange={(e) => setPurchaseTotal(maskCurrency(e.target.value))} required />
              </div>
              
              <div className="modal-input-group">
                <label>Parcelas</label>
                <div className="modal-slider-row">
                  <input type="range" className="modal-slider" min="1" max="48" value={purchaseInstallments} onChange={(e) => setPurchaseInstallments(e.target.value)} />
                  <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
                    <input type="number" className="modal-slider-input" min="1" max="48" value={purchaseInstallments} onChange={(e) => setPurchaseInstallments(e.target.value)} />
                    <span style={{ position: 'absolute', right: '12px', pointerEvents: 'none', fontWeight: 600, color: 'var(--text-main)' }}>x</span>
                  </div>
                </div>
              </div>
              ...
              <div className="modal-input-group"><label>Categoria</label><div className="modal-tags">{['Eletrônicos', 'Casa', 'Assinaturas', 'Viagem', 'Educação', 'Carros', 'Outros'].map(cat => { const isSelected = purchaseCategory === cat; return ( <span key={cat} className="modal-tag" onClick={() => setPurchaseCategory(cat)} style={isSelected ? { backgroundColor: categoryColors[cat], color: '#000', fontWeight: 600 } : {}}>{cat}</span> )})}</div></div>
